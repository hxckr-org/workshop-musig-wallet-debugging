...
describe("AdvancedMultisigWallet", () => {
  describe("Wallet Creation", () => {
    ...
    it("should throw error for invalid signature requirements", () => {
      expect(() => new AdvancedMultisigWallet(3, 2)).toThrow(
        "Invalid signature requirements"
      );
      expect(() => new AdvancedMultisigWallet(0, 3)).toThrow(
        "Invalid signature requirements"
      );
      expect(() => new AdvancedMultisigWallet(-1, 3)).toThrow(
        "Invalid signature requirements"
      );
    });
  });
  ...
  describe("Mnemonic Recovery", () => {
    ...
    it("should recover correct key pair from mnemonic", async () => {
      ...
      const recoveredKeyPair = await wallet.restoreFromMnemonic(
        mnemonic,
        index
      );

      expect(recoveredKeyPair.mnemonic).toBe(mnemonic);
      expect(recoveredKeyPair.path).toBe(`m/48'/0'/0'/2'/${index}`);
    ...
    it("should throw error for invalid mnemonic", async () => {
      ...
      await expect(
        wallet.restoreFromMnemonic(invalidMnemonic, 0)
  ...
  describe("Wallet State", () => {
    it("should throw error when accessing addresses before initialization", () => {
      const wallet = new AdvancedMultisigWallet(2, 3);
      expect(() => wallet.getAddresses()).toThrow(
        "Wallet not initialized. Call generateWallet() first."
      );
    });
